...
int main()
{
    //新建缓冲区数组与RingFifo操作句柄
    uint8_t Fifo[Read_Fifo_SIZE] ;
    ring_Fifo RB ;
    
    //初始化RingFifo操作句柄，绑定缓冲区数组；
 ...
 * 2021.06.29 v1.3.2 修复了 Ring_Fifo_Write_String 的参数类型错误
                    修复了 Ring_Fifo_Write_Byte 无法写数组最后一位的问题
 * 2022.02.17 v2.0.0 更改了delete函数的参数类型，原类型uint8_t,更改为uint32_t
                    添加了C++类，在c源代码基础上封装对应功能，代码全部在.h文件内下方
*/


/**
 * \brief 初始化新缓冲区
 * \param[out] ring_Fifo_handle: 待初始化的缓冲区结构体句柄
 * \param[in] Fifo_addr: 外部定义的缓冲区数组，类型必须为 uint8_t
 * \param[in] Fifo_size: 外部定义的缓冲区数组空间
 * \return 返回缓冲区初始化的结果
 *      \arg RING_FIFO_SUCCESS: 初始化成功
 *      \arg RING_FIFO_ERROR: 初始化失败
*/
uint8_t Ring_Fifo_Init(ring_Fifo *ring_Fifo_handle, uint8_t *Fifo_addr ,uint32_t Fifo_size)
{
    ring_Fifo_handle->head = 0 ;//复位头指针
    ring_Fifo_handle->tail = 0 ;//复位尾指针
    ring_Fifo_handle->lenght = 0 ;//复位已存储数据长度
    ring_Fifo_handle->array_addr = Fifo_addr ;//缓冲区储存数组基地址
    ring_Fifo_handle->max_lenght = Fifo_size ;//缓冲区最大可储存数据量
    if(ring_Fifo_handle->max_lenght < 2)//缓冲区数组必须有两个元素以上
        return RING_FIFO_ERROR ;//缓冲区数组过小，队列初始化失败
    else
        return RING_FIFO_SUCCESS ;//缓冲区初始化成功
}

/**
 * \brief 从头指针开始删除指定长度的数据
 * \param[out] ring_Fifo_handle: 缓冲区结构体句柄
 * \param[in] lenght: 要删除的长度
 * \return 返回删除指定长度数据结果
 *      \arg RING_FIFO_SUCCESS: 删除成功
 *      \arg RING_FIFO_ERROR: 删除失败
*/
uint8_t Ring_Fifo_Delete(ring_Fifo *ring_Fifo_handle, uint32_t lenght)
{
    if(ring_Fifo_handle->lenght < lenght)
        return RING_FIFO_ERROR ;//已储存的数据量小于需删除的数据量
    else
    {
        if((ring_Fifo_handle->head + lenght) >= ring_Fifo_handle->max_lenght)
            ...
            ring_Fifo_handle->head += lenght ;//头指针向前推进，抛弃数据
        ring_Fifo_handle->lenght -= lenght ;//重新记录有效数据长度
        return RING_FIFO_SUCCESS ;//已储存的数据量小于需删除的数据量
    }
}
 ...
 * \return 返回缓冲区写字节的结果
 *      \arg RING_FIFO_SUCCESS: 写入成功
 *      \arg RING_FIFO_ERROR: 写入失败
*/
uint8_t Ring_Fifo_Write_Byte(ring_Fifo *ring_Fifo_handle, uint8_t data)
{
    ...
    {
        *(ring_Fifo_handle->array_addr + ring_Fifo_handle->tail) = data;//基地址+偏移量，存放数据
        ring_Fifo_handle->lenght ++ ;//数据量计数+1
        ring_Fifo_handle->tail ++ ;//尾指针后移
    }
...
/**
 * \brief 从缓冲区头指针读取一个字节
 * \param[in] ring_Fifo_handle: 缓冲区结构体句柄
 * \return 返回读取的字节
*/
uint8_t Ring_Fifo_Read_Byte(ring_Fifo *ring_Fifo_handle)
{
    uint8_t data ;
    if (ring_Fifo_handle->lenght != 0)//有数据未读出
    {
        data = *(ring_Fifo_handle->array_addr + ring_Fifo_handle->head);//读取数据
...
uint8_t Ring_Fifo_Write_String(ring_Fifo *ring_Fifo_handle, uint8_t *input_addr, uint32_t write_lenght)
{
    ...
    else
    {
        ...
        {
            write_size_a = ring_Fifo_handle->max_lenght - ring_Fifo_handle->tail ;//从尾指针开始写到储存数组末尾
        ...
        if(write_size_b != 0)//需要写入两次
        {
            ...
            ring_Fifo_handle->lenght += write_lenght ;//记录新存储了多少数据量
            ring_Fifo_handle->tail = write_size_b ;//重新定位尾指针位置
        }
        else//只需写入一次
        {
            memcpy(ring_Fifo_handle->array_addr + ring_Fifo_handle->tail, input_addr, write_size_a);
            ring_Fifo_handle->lenght += write_lenght ;//记录新存储了多少数据量
            ring_Fifo_handle->tail += write_size_a ;//重新定位尾指针位置
            if(ring_Fifo_handle->tail == ring_Fifo_handle->max_lenght)
                ring_Fifo_handle->tail = 0 ;//如果写入数据后尾指针刚好写到数组尾部，则回到开头，防止越位
        }
        return RING_FIFO_SUCCESS ;
    }
}
 ...
 *      \arg RING_FIFO_SUCCESS: 读取成功
 *      \arg RING_FIFO_ERROR: 读取失败
*/
uint8_t Ring_Fifo_Read_String(ring_Fifo *ring_Fifo_handle, uint8_t *output_addr, uint32_t read_lenght)
{
    ...
    else
    {
        ...
        if(Read_size_b != 0)//需要读取两次
        {
            ...
            ring_Fifo_handle->lenght -= read_lenght ;//记录剩余数据量
            ring_Fifo_handle->head = Read_size_b ;//重新定位头指针位置
        }
        else
        {
            memcpy(output_addr, ring_Fifo_handle->array_addr + ring_Fifo_handle->head, Read_size_a);
            ring_Fifo_handle->lenght -= read_lenght ;//记录剩余数据量
            ring_Fifo_handle->head += Read_size_a ;//重新定位头指针位置
            if(ring_Fifo_handle->head == ring_Fifo_handle->max_lenght)
                ring_Fifo_handle->head = 0 ;//如果读取数据后头指针刚好写到数组尾部，则回到开头，防止越位
        }
        return RING_FIFO_SUCCESS ;
    }
}
 ...
 * \return 返回插入关键词的结果
 *      \arg RING_FIFO_SUCCESS: 插入成功
 *      \arg RING_FIFO_ERROR: 插入失败
*/
 ...
 * \param[in] keyword_lenght:关键词字节数，最大4字节（32位）
 * \return 返回获取匹配字符最高位需要读取的字节数，返回 0/RING_FIFO_ERROR: 则查找失败
*/
uint32_t Ring_Fifo_Find_Keyword(ring_Fifo *ring_Fifo_handle, uint32_t keyword, uint8_t keyword_lenght)
{
    uint32_t max_find_lenght = ring_Fifo_handle->lenght - keyword_lenght + 1 ;//计算需要搜索的最大长度
    uint8_t trigger_word = keyword >> ((keyword_lenght - 1) * 8) ;//计算触发关键词检查的字节（最高位）
    uint32_t distance = 1 , find_head = ring_Fifo_handle->head;//记录关键词距离头指针的长度/临时头指针获取原头指针初始值
    while(distance <= max_find_lenght)//在设定范围内搜索关键词（防止指针越位错误）
    {
        if(*(ring_Fifo_handle->array_addr + find_head) == trigger_word)//如果高位字节匹配则开始向低位检查
            if(Ring_Fifo_Get_Word(ring_Fifo_handle, find_head, keyword_lenght) == keyword)//满足关键词匹配
                return distance ;//返回长度，使用 Ring_Fifo_Read_String 可提取数据
